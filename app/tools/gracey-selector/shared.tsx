...
import { GraceySelector } from "../../_components/GraceySelector";
import { ToolShell } from "../../_components/ToolShell";
import { localizedPath, type SiteLocale } from "../../_lib/catalog";
import { graceyStrings } from "../_strings/gracey-selector";
import "./selector.css";

const TOOL_PATH = "/tools/gracey-selector";
...
export function buildMetadata(locale: SiteLocale): Metadata {
  const { title, description } = graceyStrings[locale].meta;
...
export function GraceySelectorPage({ locale }: { locale: SiteLocale }) {
  const s = graceyStrings[locale];

  return (
    <ToolShell locale={locale} labels={s.shell}>
      <section className="section">
        <GraceySelector locale={locale} />
      </section>
